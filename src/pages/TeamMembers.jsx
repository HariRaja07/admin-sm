...
const TeamMembers = () => {
  ...
  const handleDelete = async (id) => {
    const response = await fetch(`${backendUrl}/api/team-members/${id}`, {
      method: "DELETE",
    });
    if (response.ok) {
    ...
    <div className="container mx-auto px-4 py-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {members.map((member) => (
          <div
            key={member._id}
            className="shadow-xl border border-blue-200 bg-blue-300 rounded-lg overflow-hidden transition-transform transform hover:scale-105"
          >
      ...
      >
        <form onSubmit={form.id ? handleEditSubmit : handleAddSubmit}>
          ...
          <button
            type="submit"
            className="bg-blue-600 text-white font-semibold py-2 px-4 rounded w-full"
          >
            Save
          </button>
        </form>
